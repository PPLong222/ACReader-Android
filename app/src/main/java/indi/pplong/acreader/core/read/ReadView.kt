...
import indi.pplong.acreader.feature.read.ui.ReadViewUiIntent.OnReadViewResize
...
@Composable
fun ReadView(uiState: ReadViewUiState, sendIntent: (ReadViewUiIntent) -> Unit) {

    BasicReadView(chapterName = uiState.chapterTitle, dataLine = uiState.contentPage, onSize = { size ->
        sendIntent(OnReadViewResize(size))
    }) {
        sendIntent(ReadViewUiIntent.NextPage)
    }
